...
class WavDataset(Dataset):
    ...
    def __init__(self, opt=None, train_mode=True):
        ...
        if train_mode:
            ...
            # self.utt2spk = {line.split()[0]:opt.spk2int[line.split()[1]] for line in open('%s/mic_utt2spk' % opt.train_dir)}
            self.utt2spk = {line.split()[0]:opt.spk2int[line.split()[1]] for line in open('%s/train_all_utt2spk' % opt.train_dir)}
    ...
    def __getitem__(self, idx):
        ...
        filename = filename.replace('智能家居场景说话人识别挑战赛','teamshare')
