...
from model.resnet import StatsPool
from model.se_block import SEBlock
import torch.utils.checkpoint as checkpoint

from model.tdnn import ASP
...
#####
class RepVGGStatsPool(nn.Module):
    def __init__(self, in_planes, embedding_size, dropout=0.5, **kwargs):

        super(RepVGGStatsPool, self).__init__()
        self.front = get_RepVGG_func_by_name(name='RepVGG-A0')()
        # self.pool = ASP(input_dim=int(in_planes*8*2.75*10))
        # self.pool = MQMHASTP(in_dim=in_planes*8*2.75*10)
        self.pool = MQMHASTP(in_dim=1280*10)
        # self.bottleneck = nn.Linear(int(in_planes*8*2.75*10*2*2), embedding_size)
        self.bottleneck = nn.Linear(int(1280*10*2*2), embedding_size)
        self.drop = nn.Dropout(dropout) if dropout else None
        
    def forward(self, x):
        # x = self.relu(self.bn1(self.conv1(x.unsqueeze(dim=1))))
        x = self.front(x.unsqueeze(dim=1))
        x = self.pool(x.view(x.shape[0], x.shape[1]*x.shape[2], -1))
        x = self.bottleneck(x)
        if self.drop:
            x = self.drop(x)
        return x
    
class RepVGGStatsPool_(nn.Module):
    def __init__(self, in_planes, embedding_size, dropout=0.5, **kwargs):

        super(RepVGGStatsPool_, self).__init__()
        self.front = get_RepVGG_func_by_name(name='RepVGG-A0')()
        self.pool = ASP(input_dim=int(12800))
        # self.pool = MQMHASTP(in_dim=in_planes*8*2.75*10)
        # self.pool = MQMHASTP(in_dim=12800)
        # self.bottleneck = nn.Linear(int(in_planes*8*2.75*10*2*2), embedding_size)
        # self.bottleneck = nn.Linear(int(12800*2*2), embedding_size)
        self.bottleneck = nn.Linear(int(25600), embedding_size)
        self.drop = nn.Dropout(dropout) if dropout else None
