...
class ECAPA_TDNN(nn.Module):
    ...
    def __init__(self,in_dim=80,hidden_dim=512,scale=8,bottleneck=128,embedding_size=192,asp='asp'):
        ...
        if asp=='asp':
            ...
            self.ASP = MQMHASTP(in_dim=hidden_dim*3)
            self.bn2 = nn.BatchNorm1d(self.hidden_dim*3*2*2)
        
            self.fc = nn.Linear(self.hidden_dim*3*2*2,self.embedding_size)
