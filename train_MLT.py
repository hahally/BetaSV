...
def main():
    ...
    # validation dataset
    val_dataset = MLTDataset(opt=opt, train_mode=False)
    val_dataloader = DataLoader(val_dataset, num_workers=opt.workers, pin_memory=True, batch_size=1)

    if opt.conv_type == '1D':
        model = getattr(MLTmodel, opt.model)(in_dim=opt.in_planes, embedding_size=opt.embd_dim, hidden_dim=opt.hidden_dim).cuda() # tdnn, ecapa_tdnn
    else:
        exit(0)
